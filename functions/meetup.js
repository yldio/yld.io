...
/******/ 	return __webpack_require__(__webpack_require__.s = "./meetup.js");
/******/ })
/************************************************************************/
/******/ ({
...
const {
  createClient
} = __webpack_require__(/*! contentful-management */ "contentful-management");

const {
  default: Map
} = __webpack_require__(/*! apr-map */ "apr-map");
...
const isEqual = __webpack_require__(/*! lodash.isequal */ "lodash.isequal"); // Set up dot-env variables
...
const generateContentfulEvent = ({
  ...
  fields: {
    ...
    address: {
      'en-US': venue !== 'Venue To Be Confirmed' ? `${venue.name}&&${venue.address1}&&${venue.adress2 ? venue.adress2 : ''}&&${venue.address3 ? venue.address3 : ''}&&${venue.city}` : 'Venue To Be Confirmed'
    },
    eventTitle: {
      'en-US': eventName
    },
...
const processMeetupEvent = eventObject => {
  let outputObject = {
    ...
    description: eventObject.description.includes('EVENT SUMMARY') && eventObject.description.includes('EVENT DETAILS') ? striptags(eventObject.description.split('EVENT SUMMARY:')[1].split('EVENT DETAILS')[0]).trim() : 'For more information, please visit the Meetup page'
  };
  return outputObject;
...
exports.handler = async (event, context, callback) => {
  ...
  await Map(processMeetupData((await getSelfGroups())), async (group, index) => {
    ...
    const meetup = processMeetupEvent((await getEvent({
      id: nextEvent,
      urlname
    })));
    const contentfulEvent = find(events, ['fields.linkToEvent.en-US', meetup.link]);
    ...
    if (generatedEvent) {
      ...
      const diffVals = Object.keys(generatedEventFields).reduce((acc, curr) => {
        ...
        if (['startTime', 'endTime'].includes(curr)) {
          return new Date(contentfulEventFields[curr]['en-US']) - generatedEventFields[curr]['en-US'] === 0 ? acc : [...acc, curr];
        }

        return isEqual(contentfulEventFields[curr], generatedEventFields[curr]) ? acc : [...acc, curr];
      }, []); // If there are no differences then length will be 0

      if (diffVals && !diffVals.length) {
        console.log(`Entry ${meetup.eventName} unchanged. No need to update. Moving on.`);
        return;
      } // update


      contentfulEvent.fields = Object.assign(contentfulEvent.fields, diffVals.reduce((acc, curr) => ({ ...acc,
      ...
      if (isProd) {
        ...
        console.log(`Not prod so not updating contentful for ${meetup.eventName}`);
        return;
      }
    } // create
    ...
    if (isProd) {
      ...
      console.log(`Not prod so not creating contentful event for ${meetup.eventName}`);
      return;
    }
  });
  return {
